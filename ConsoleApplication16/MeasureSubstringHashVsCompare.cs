namespace ConsoleApplication16
{
    ...
    class MeasureSubstringHashVsCompare
    {
        public static long Run()
        {
            ...
            {
                var matches = Enumerable.Range(1, matchCount).Select(i => new SubString(Value, i, matchLength)).Select(x => Tuple.Create(x, 1)).ToArray();
    ...
    public class SubStringTree
    {
        ...
        {
            root = new Node(this.comparer);
            this.MapTree(root, strings);
        ...
        void Optimize(SubString key, Node current, Node parent)
        {
            ...
            {
                var pair = current.Children.First();
                var node = pair.Value;
        ...
        void MapTree(Node current, IEnumerable<KeyValuePair<SubString, int>> strings)
        {
            ...
            var charGroups = (
                from s in strings where s.Key.Length > 0
                ...
                .ToDictionary(
                    ...
                    g => 
                    {
                        ...
                        node.PrefixLength = 1;
                        var remainder = g.ToArray();
                        var ownValue = remainder.Where(x => x.Key.Length == 0).Select(x => x.Value).DefaultIfEmpty(-1).First();
                        if (ownValue > -1)
                        {
                            node.Value = ownValue;
                        }
        ...
        class Node
        {
            ...
            {
                Children = new SortedList<SubString, Node>(comparer);
            }
        ...
        class FixedLengthComparer : IComparer<SubString>
        {
            ...
            public int Compare(SubString x, SubString y)
            {
                if (Enabled)
                {
                    return innerComparer.Compare(x, y.Substring(0, Math.Min(y.Length, x.Length)));
                }
